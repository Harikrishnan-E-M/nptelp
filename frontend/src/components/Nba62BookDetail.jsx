...
/**
 * Nba62BookDetail — fetches nba62BookData records for a specific year document.
 * Columns: S.No | Faculty Name | Authors | Paper Title | Title of the Book/Book Chapter | Published | Link
 */
function Nba62BookDetail({ parentDocId, yearLabel, onBack }) {
  ...
  const fetchRows = async () => {
    try {
      setLoading(true);
      const query = `*[_type == "nba62BookData" && parent._ref == $parentDocId] | order(sNo asc) {
        _id,
        sNo,
        facultyName,
        authors,
      ...
      setError('Failed to load 6.2 Book data.');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const getSortedRows = () => {
    ...
    else if (sortBy === 'book')    sorted.sort((a, b) => (a.bookTitle || '').localeCompare(b.bookTitle || ''));
    return sorted;
  };

  if (loading) return <div className="alert alert-info">Loading 6.2 Book data...</div>;
  ...
  return (
    <div>
      ...
      <div className="d-flex align-items-center mb-2">
        ...
        <h4 className="mb-0">6.2 Book — {yearLabel}</h4>
      </div>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
          <label className="modal-sort-label" htmlFor="nba62book-sort" style={{ margin: 0, whiteSpace: 'nowrap' }}>
          ...
          <table className="table table-bordered table-hover align-middle text-center">
            <thead className="table-dark">
              <tr>
                ...
                <th className="text-start">Book / Book Chapter Title</th>
                <th>Published</th>
                <th>Link</th>
              </tr>
            </thead>
            <tbody>
              {sortedRows.map((row, idx) => (
                <tr key={row._id}>
                  ...
                  <td className="text-start">{sanitize(row.bookTitle) || '—'}</td>
...
export default Nba62BookDetail;
